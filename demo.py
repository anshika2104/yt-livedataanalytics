...
total_subscriber_count = filtered_data['total_subscriber_count'].sum()
total_views_count = filtered_data['total_views_count'].sum()
total_video_count= filtered_data['total_video_count'].sum()
...
with col1:
    st.subheader(' total_subscriber_count')
    st.metric("Subscribers", f"{total_subscriber_count/1000:.1f}K")

# Column 2: Total Views
with col2:
    st.subheader('totalchannel_views_count')
    st.metric("total_views_count", f"{total_views_count/1000:.1f}K")

# Column 3: total_video_count
with col3:
    st.subheader('total_video_count')
    st.metric("Likes", f"{total_video_count/1000:.1f}K")
...
# Area chart: total_views_count Over Video Title
st.subheader('Area Chart: total_views_count Over Video Title')
fig_area = go.Figure()
for video_title in filtered_data['title_name'].unique():
    video_data = filtered_data[filtered_data['title_name'] == video_title]
    fig_area.add_trace(go.Scatter(x=video_data['title_name'], y=video_data['total_views_count'],
                                  mode='lines', name=video_title, fill='tozeroy'))
fig_area.update_layout(xaxis_title='title_name', yaxis_title='total_views_count',
                       title=f'Area total_views_count Over Video Title - {selected_playlist}',
                       template='plotly_dark')  # Use dark template for a sleek look
fig_area.update_xaxes(showline=True, linewidth=2, linecolor='black', gridcolor='rgba(255, 255, 255, 0.1)')
fig_area.update_yaxes(showline=True, linewidth=2, linecolor='black', gridcolor='rgba(255, 255, 255, 0.1)')
